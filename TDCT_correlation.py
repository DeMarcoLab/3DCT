...
class MainWidget(QtGui.QMainWindow, Ui_WidgetWindow):
    def __init__(self, parent=None, leftImage=None, rightImage=None, workingdir=None):
        ...
        self.toolButton_resetLeftImage.clicked.connect(
            lambda: self.resetImageLeft(img=None)
        )
        self.toolButton_resetRightImage.clicked.connect(
            lambda: self.resetImageRight(img=None)
        )
        ...
        self.tableView_left.selectionModel().selectionChanged.connect(
            self.tableView_left.showSelectedItem
        )
        self.tableView_right.selectionModel().selectionChanged.connect(
            self.tableView_right.showSelectedItem
        )
        self.tableView_results.selectionModel().selectionChanged.connect(
            self.showSelectedResidual
        )
        self.tableView_results.doubleClicked.connect(
            lambda: self.showSelectedResidual(doubleclick=True)
        )
        ...
        ## Comboboxes
        self.comboBox_channelColorLayer1.currentIndexChanged.connect(
            self.changeColorChannel
        )
        self.comboBox_channelColorLayer2.currentIndexChanged.connect(
            self.changeColorChannel
        )
        self.comboBox_channelColorLayer3.currentIndexChanged.connect(
            self.changeColorChannel
        )
        ...
        ## Buttons
        self.toolButton_rotcw.clicked.connect(
            lambda: self.rotateImage45(direction="cw")
        )
        self.toolButton_rotccw.clicked.connect(
            lambda: self.rotateImage45(direction="ccw")
        )
        self.toolButton_brightness_reset.clicked.connect(
            lambda: self.horizontalSlider_brightness.setValue(0)
        )
        self.toolButton_contrast_reset.clicked.connect(
            lambda: self.horizontalSlider_contrast.setValue(10)
        )
        ...
        self.toolButton_saveImage_left.clicked.connect(
            lambda: self.displayImage(side="left", save=True)
        )
        self.toolButton_saveImage_right.clicked.connect(
            lambda: self.displayImage(side="right", save=True)
        )
        self.toolButton_loadLayer2.clicked.connect(
            lambda: self.layerCtrl("layer2", load=True)
        )
        self.toolButton_loadLayer3.clicked.connect(
            lambda: self.layerCtrl("layer3", load=True)
        )
        ...
        self.tableView_results.customContextMenuRequested.connect(
            self.cmTableViewResults
        )
    ...
    def selectWorkingDir(self):
        path = str(
            QtGui.QFileDialog.getExistingDirectory(
                self, "Select working directory", self.workingdir
            )
        )
        self.activateWindow()
        if path:
    ...
    def updateWorkingDir(self):
        if os.path.isdir(self.lineEdit_workingDir.text()):
            workingdir = self.checkWorkingDirPrivileges(
                str(self.lineEdit_workingDir.text())
            )
            if workingdir:
                self.workingdir = workingdir
            self.lineEdit_workingDir.setText(self.workingdir)
            ...
            print(
                clrmsg.ERROR
                + "Dropped object is not a valid path. Returning to {0} as working directory.".format(
                    self.workingdir
                )
            )
    ...
    def checkWorkingDirPrivileges(self, path):
        try:
            ...
            QtGui.QMessageBox.critical(
                self,
                "Warning",
                "I cannot write to this folder: {0}\nFalling back to {1} as the working directory".format(
    ...
    def changedFocusSlot(self, former, current):
        if debug is True:
            print(
                clrmsg.DEBUG + "focus changed from/to:",
        ...
        if self.currentFocusedWidgetName in [
            ...
            "spinBox_markerSize",
            "spinBox_slice",
            "horizontalSlider_brightness",
            "horizontalSlider_contrast",
            "doubleSpinBox_custom_rot_center_x",
            "doubleSpinBox_custom_rot_center_y",
            "doubleSpinBox_custom_rot_center_z",
            "checkBox_MIP",
            "checkBox_layer1",
            "checkBox_layer2",
            "checkBox_layer3",
            "comboBox_channelColorLayer1",
            "comboBox_channelColorLayer2",
            "comboBox_channelColorLayer3",
            "radioButton_layer1",
            "radioButton_layer2",
            "radioButton_layer3",
            "",
            ...
            if (
                self.currentFocusedWidgetName != "graphicsView_left"
                and self.currentFocusedWidgetName != "graphicsView_right"
            ):
        ...
        if (
            self.currentFocusedWidgetName != "tableView_left"
            and self.currentFocusedWidgetName != "tableView_right"
        ):
        ...
        if self.currentFocusedWidgetName == "graphicsView_left":
            ...
            self.label_imgpxsize.setText(
                str(self.sceneLeft.pixelSize)
            )  # + ' um') # breaks marker size adjustments check
            self.label_imgpxsizeUnit.setText(
                "um"
            ) if self.sceneLeft.pixelSize else self.label_imgpxsizeUnit.setText("")
        elif self.currentFocusedWidgetName == "graphicsView_right":
            ...
            self.label_imgpxsize.setText(
                str(self.sceneRight.pixelSize)
            )  # + ' um') # breaks marker size adjustments check
            self.label_imgpxsizeUnit.setText(
                "um"
            ) if self.sceneRight.pixelSize else self.label_imgpxsizeUnit.setText("")
    ...
    def borderControl(self, widget, idx=0):
        if idx == 0:
            ...
            self.graphicsView_right.setStyleSheet(
    ...
    def colorModels(self):
        ...
        for row in range(min([rowsLeft, rowsRight])):
            ...
            if rowsLeft != 0:
                try:
                    self.modelLleft.item(row, 0).setBackground(
                        QtGui.QColor(*color_correlate)
                    )
                    self.modelLleft.item(row, 1).setBackground(
                        QtGui.QColor(*color_correlate)
                    )
                    self.modelLleft.item(row, 2).setBackground(
                        QtGui.QColor(*color_correlate)
                    )
                except:
                    if debug is True:
            ...
            if rowsRight != 0:
                try:
                    self.modelRight.item(row, 0).setBackground(
                        QtGui.QColor(*color_correlate)
                    )
                    self.modelRight.item(row, 1).setBackground(
                        QtGui.QColor(*color_correlate)
                    )
                    self.modelRight.item(row, 2).setBackground(
                        QtGui.QColor(*color_correlate)
                    )
                except:
                    if debug is True:
        ...
        if rowsLeft > rowsRight:
            if (
                "{0:b}".format(self.sceneLeft.imagetype)[-1] == "0"
                or "{0:b}".format(self.sceneLeft.imagetype)[-1]
                == "{0:b}".format(self.sceneRight.imagetype)[-1]
            ):
                color_overflow = (
                    105,
                    220,
                    0,
                    alpha,
                )  # green if entries are used as POIs
            else:
                color_overflow = (
                    220,
                    25,
                    105,
                    alpha,
                )  # red(ish) color to indicate unbalanced amount of markers for correlation
            for row in range(rowsRight, rowsLeft):
                try:
                    self.modelLleft.item(row, 0).setBackground(
                        QtGui.QColor(*color_overflow)
                    )
                    self.modelLleft.item(row, 1).setBackground(
                        QtGui.QColor(*color_overflow)
                    )
            ...
            if (
                "{0:b}".format(self.sceneRight.imagetype)[-1] == "0"
                or "{0:b}".format(self.sceneLeft.imagetype)[-1]
                == "{0:b}".format(self.sceneRight.imagetype)[-1]
            ):
                color_overflow = (
    ...
    def getMarkerColor(self):
        ...
        if color.isValid():
            ...
            self.label_markerColor.setStyleSheet(
                "background-color: rgb{0};".format(
    ...
    def getPoiColor(self):
        ...
        if color.isValid():
            ...
            self.label_poiColor.setStyleSheet(
                "background-color: rgb{0};".format(
                    (color.red(), color.green(), color.blue())
                )
            )
    ...
    def initImageLeft(self):
        if self.leftImage is not None:
            ...
            try:
                splashscreen.splash.showMessage(
                    "Loading images... " + self.leftImage, color=QtCore.Qt.white
                )
            ...
            self.img_left_layer1, self.sceneLeft.imagetype, self.imgstack_left_layer1 = self.imread(
                self.leftImage
            )
            ...
            self.pixmap_item_left = QtGui.QGraphicsPixmapItem(
                self.pixmap_left, None, self.sceneLeft
            )
            ...
            scaling_factor = float(self.size) / max(
                self.pixmap_left.width(), self.pixmap_left.height()
            )
    ...
    def initImageRight(self):
        if self.rightImage is not None:
            ...
            try:
                splashscreen.splash.showMessage(
                    "Loading images... " + self.rightImage, color=QtCore.Qt.white
                )
            except Exception as e:
                print(clrmsg.WARNING, e)
                pass
            ...
            ## Load image, assign it to scene and store image type information
            self.img_right_layer1, self.sceneRight.imagetype, self.imgstack_right_layer1 = self.imread(
                self.rightImage
            )
            ...
            self.pixmap_item_right = QtGui.QGraphicsPixmapItem(
                self.pixmap_right, None, self.sceneRight
            )
            ## connect scenes to GUI elements
            ...
            ## scaling scene, not image
            scaling_factor = float(self.size) / max(
                self.pixmap_right.width(), self.pixmap_right.height()
            )
    ...
    def openImageLeft(self):
        ...
        path = str(
            QtGui.QFileDialog.getOpenFileName(
                None,
                "Select image file for correlation",
                self.workingdir,
        ...
        if path != "":
            ...
            self.img_left_layer2, self.img_adj_left_layer2, self.sceneLeft.imagetype_layer2, self.imgstack_left_layer2 = (
                None,
                None,
                None,
                None,
            )
            self.img_left_layer3, self.img_adj_left_layer3, self.sceneLeft.imagetype_layer3, self.imgstack_left_layer3 = (
                None,
                None,
                None,
                None,
    ...
    def openImageRight(self):
        ...
        if path != "":
            ...
            self.spinBox_slice.setValue(0)
            self.checkBox_MIP.setChecked(True)
            self.img_right_layer2, self.img_adj_right_layer2, self.sceneRight.imagetype_layer2, self.imgstack_right_layer2 = (
                None,
                None,
                None,
                None,
            )
            self.img_right_layer3, self.img_adj_right_layer3, self.sceneRight.imagetype_layer3, self.imgstack_right_layer3 = (
                None,
                None,
                None,
                None,
    ...
    def rotateImage(self):
        if self.label_selimg.text() == "left":
            ...
            self.graphicsView_left.rotate(
                int(self.spinBox_rot.value()) - self.sceneLeft.rotangle
            )
            ...
            if int(self.spinBox_rot.value()) == 360:
                ...
                self.spinBox_rot.setValue(359)
            self.graphicsView_right.rotate(
                int(self.spinBox_rot.value()) - self.sceneRight.rotangle
            )
    ...
    def rotateImage45(self, direction=None):
        ...
        # rotate 45 degree clockwise
            ...
            if self.label_selimg.text() == "left":
                ...
                self.sceneRight.rotangle = self.anglectrl(
                    angle=self.sceneRight.rotangle
                )
                self.spinBox_rot.setValue(self.sceneRight.rotangle)
                ## Update graphics
                self.sceneRight.enumeratePoints()
    ...
    def anglectrl(self, angle=None):
        if angle is None:
            print(
                clrmsg.ERROR
                + "Please specify side, e.g. anglectrl(angle=self.sceneLeft.rotangle)"
            )
        elif angle >= 360:
            angle -= 360
        elif angle < 0:
    ...
    def changeMarkerSize(self):
        if self.label_selimg.text() == "left":
            ...
            if self.sceneLeft.pixelSize:
                if debug is True:
                    print(
                        clrmsg.DEBUG + "Doing stuff with image pixelSize (left image).",
                        self.label_imgpxsize.text(),
                    )
                try:
                    self.label_markerSizeNano.setText(
                        str(self.sceneLeft.markerSize * 2 * self.sceneLeft.pixelSize)
                    )
                    self.label_markerSizeNanoUnit.setText(self.sceneLeft.pixelSizeUnit)
                except:
                    if debug is True:
                        print(
                            clrmsg.DEBUG + "Image pixel size is not a number:",
                            self.label_imgpxsize.text(),
                        )
            ...
            if self.sceneRight.pixelSize:
                if debug is True:
                    print(
                        clrmsg.DEBUG
                        + "Doing stuff with image pixelSize (right image).",
                        self.label_imgpxsize.text(),
                    )
                try:
                    self.label_markerSizeNano.setText(
                        str(self.sceneRight.markerSize * 2 * self.sceneRight.pixelSize)
                    )
    ...
    def imread(self, path, normalize=True):
        ...
        ## this can only handle rgb. For more channels set "3" to whatever max number of channels should be handled
            ...
            img.ndim == 3
            and any([True for dim in img.shape if dim <= 4])
            or img.ndim == 2
        ):
            if debug is True:
                print(
                    clrmsg.DEBUG + "Loading regular 2D image... multicolor/normalize:",
                    [True for x in [img.ndim] if img.ndim == 3],
                    "/",
                    [normalize],
                )
            ...
            for page in tif:
                for tag in list(page.tags.values()):
                    if isinstance(tag.value, str):
                        for keyword in (
                            ["PhysicalSizeX", "PixelWidth", "PixelSize"]
                            if not z
                            else ["PhysicalSizeZ", "FocusStepSize"]
                        ):
                            tagposs = [
                                m.start() for m in re.finditer(keyword, tag.value)
                            ]
                            for tagpos in tagposs:
                                if (
                                    keyword == "PhysicalSizeX"
                                    or keyword == "PhysicalSizeZ"
                                ):
                                    for piece in tag.value[tagpos : tagpos + 30].split(
                                        ...
                                        try:
                                            ...
                                            if debug is True:
                                                print(
                                                    clrmsg.DEBUG + "Pixel size parser:",
                                                    e,
                                                )
                                            pass
        ...
        if self.label_selimg.text() == "left":
            if self.radioButton_layer1.isChecked():
                ...
                self.img_adj_left_layer1 = self.adjustBrightCont(
                    self.img_left_displayed_layer1,
                    self.img_adj_left_layer1,
                    self.brightness_left_layer1,
                    self.contrast_left_layer1,
                )
            elif self.radioButton_layer2.isChecked():
                ...
                self.img_adj_left_layer2 = self.adjustBrightCont(
                    self.img_left_displayed_layer2,
                    self.img_adj_left_layer2,
                    self.brightness_left_layer2,
                    self.contrast_left_layer2,
                )
            elif self.radioButton_layer3.isChecked():
                ...
                self.img_adj_left_layer3 = self.adjustBrightCont(
                    self.img_left_displayed_layer3,
                    self.img_adj_left_layer3,
                    self.brightness_left_layer3,
                    self.contrast_left_layer3,
                )
        if self.label_selimg.text() == "right":
            if self.radioButton_layer1.isChecked():
                ...
                self.img_adj_right_layer1 = self.adjustBrightCont(
                    self.img_right_displayed_layer1,
                    self.img_adj_right_layer1,
                    self.brightness_right_layer1,
                    self.contrast_right_layer1,
                )
            elif self.radioButton_layer2.isChecked():
                ...
                self.img_adj_right_layer2 = self.adjustBrightCont(
                    self.img_right_displayed_layer2,
                    self.img_adj_right_layer2,
                    self.brightness_right_layer2,
                    self.contrast_right_layer2,
                )
            elif self.radioButton_layer3.isChecked():
                ...
                self.img_adj_right_layer3 = self.adjustBrightCont(
                    self.img_right_displayed_layer3,
                    self.img_adj_right_layer3,
                    self.brightness_right_layer3,
                    self.contrast_right_layer3,
                )
        ## Display image
        self.displayImage()
    ...
    def adjustBrightCont(self, img_displayed, img_adjusted, brightness, contrast):
        ...
        ## Adjust brightness
        if brightness > 0:
            img_adjusted = np.where(
                255 - img_adjusted <= brightness, 255, img_adjusted + brightness
            )
        else:
            img_adjusted = np.where(
                img_adjusted <= -brightness, 0, img_adjusted + brightness
            )
    ...
    def norm_img(self, img, copy=False):
        ...
        ## 3D or multichannel image
            ...
            if img.shape[-1] > 4:
                if debug is True:
                    print(clrmsg.DEBUG + "image stack")
                for i in range(int(img.shape[0])):
                    img[i, :, :] *= typesize / img[i, :, :].max()
            else:
                if debug is True:
                    print(clrmsg.DEBUG + "multichannel image")
    ...
    def selectSlice(self):
        ...
        if self.checkBox_MIP.isChecked():
            if (
                self.label_selimg.text() == "left"
                and "{0:b}".format(self.sceneLeft.imagetype)[-1] == "0"
            ):
                ...
                self.label_selimg.text() == "right"
                and "{0:b}".format(self.sceneRight.imagetype)[-1] == "0"
            ):
                ...
                if (
                    self.brightness_right_layer1 != 0
                    or self.contrast_right_layer1 != 10
                ):
                    self.setBrightCont()
                else:
            ...
            if (
                ...
                # img = self.imgstack_left_layer1[self.slice_left,:]
                self.img_left_displayed_layer1 = self.imgstack_left_layer1[
                    self.slice_left, :
                ]
                self.img_adj_left_layer1 = self.imgstack_left_layer1[self.slice_left, :]
                if self.img_left_layer2 is not None:
                    self.img_left_displayed_layer2 = self.imgstack_left_layer2[
                        self.slice_left, :
                    ]
                    self.img_adj_left_layer2 = self.imgstack_left_layer2[
                        self.slice_left, :
                    ]
                if self.img_left_layer3 is not None:
                    self.img_left_displayed_layer3 = self.imgstack_left_layer3[
                        self.slice_left, :
                    ]
                    self.img_adj_left_layer3 = self.imgstack_left_layer3[
                        self.slice_left, :
                    ]
                ...
                # img = self.imgstack_right_layer1[self.slice_right,:]
                self.img_right_displayed_layer1 = self.imgstack_right_layer1[
                    self.slice_right, :
                ]
                self.img_adj_right_layer1 = self.imgstack_right_layer1[
                    self.slice_right, :
                ]
                if self.img_right_layer2 is not None:
                    self.img_right_displayed_layer2 = self.imgstack_right_layer2[
                        self.slice_right, :
                    ]
                    self.img_adj_right_layer2 = self.imgstack_right_layer2[
                        self.slice_right, :
                    ]
                if self.img_right_layer3 is not None:
                    self.img_right_displayed_layer3 = self.imgstack_right_layer3[
                        self.slice_right, :
                    ]
                    self.img_adj_right_layer3 = self.imgstack_right_layer3[
                        self.slice_right, :
                    ]
    ...
    def changeColorChannel(self):
        if self.label_selimg.text() == "left":
            ...
            if (
                self.layer1Color_right
                != self.comboBox_channelColorLayer1.currentIndex()
            ):
            ...
            if (
                self.layer2Color_right
                != self.comboBox_channelColorLayer2.currentIndex()
            ):
            ...
            if (
                self.layer3Color_right
                != self.comboBox_channelColorLayer3.currentIndex()
            ):
    ...
    def colorizeImage(self, img, color=None):
        if debug is True:
            ping = time.time()
        if color is None and all(
            comboboxColor == "none"
            for comboboxColor in [
    ...
    def displayImage(self, side=None, save=False, keepRGB=False):
        ...
        if side == "left":
            ...
            QtGui.QGraphicsItem.stackBefore(
                self.pixmap_item_left, list(self.sceneLeft.items())[-1]
            )
            ...
            ## Put exchanged image into background
            QtGui.QGraphicsItem.stackBefore(
                self.pixmap_item_right, list(self.sceneRight.items())[-1]
            )
            ## fix bug, where markers vanished behind image, by setting z value low enough
        ...
        if save is True:
            ...
            cv2.imwrite(
                os.path.join(self.workingdir, timestamp + "_image.tif"),
                ...
                if i == 0:
                    ...
                    if blendmode == "screen":
                        blend = (
                            blend
                            + images[i]
                            - (blend * images[i].astype(dtype=np.float32) / 255.0)
                        )
    ...
    def layerCtrl(self, layer, load=False):
        ...
        if layer == "layer1":
            ...
            if self.label_selimg.text() == "left":
                ...
                if (
                    self.img_left_layer2 is None
                    and self.checkBox_layer2.isChecked()
                    or load is True
                ):
                    path = str(
                        QtGui.QFileDialog.getOpenFileName(
                    ...
                    self.img_left_layer2, self.sceneLeft.imagetype_layer2, self.imgstack_left_layer2 = self.imread(
                        path
                    )
                    ...
                    if self.sceneLeft.imagetype_layer2 != self.sceneLeft.imagetype:
                        QtGui.QMessageBox.critical(
                            self,
                            "Warning",
                            "This image file does not seem to be of the same kind as the first image!",
                        )
                        self.img_left_layer2, self.sceneLeft.imagetype_layer2, self.imgstack_left_layer2 = (
                            None,
                            None,
                            None,
                        )
                        ...
                        self.imgstack_left_layer1 is not None
                        and self.imgstack_left_layer2.shape
                        != self.imgstack_left_layer1.shape
                    ):
                        QtGui.QMessageBox.critical(
                            self,
                            "Warning",
                            "This image file does not have the same dimensions as the first image: \nNeeds to be {0}\nbut is{1}".format(
                                self.imgstack_left_layer1.shape,
                                self.imgstack_left_layer2.shape,
                            ),
                        )
                        self.img_left_layer2, self.sceneLeft.imagetype_layer2, self.imgstack_left_layer2 = (
                            None,
                        ...
                        self.imgstack_left_layer1 is None
                        and self.img_left_layer2.shape != self.img_left_layer1.shape
                    ):
                        QtGui.QMessageBox.critical(
                            self,
                            "Warning",
                ...
                if (
                    ...
                    if path == "":
                        if load is True:
                            ...
                            return
                    # path = '/Users/jan/Desktop/correlation_test_dataset/single_tif_files/single_tif_files_1.tif'
                    self.img_right_layer2, self.sceneRight.imagetype_layer2, self.imgstack_right_layer2 = self.imread(
                        path
                    )
                    ...
                    if self.sceneRight.imagetype_layer2 != self.sceneRight.imagetype:
                        ...
                        self.img_right_layer2, self.sceneRight.imagetype_layer2, self.imgstack_right_layer2 = (
                            None,
                            None,
                            None,
                        )
                        ...
                        self.imgstack_right_layer1 is not None
                        and self.imgstack_right_layer2.shape
                        != self.imgstack_right_layer1.shape
                    ):
                        QtGui.QMessageBox.critical(
                            self,
                            "Warning",
                            "This image file does not have the same dimensions as the first image: \nNeeds to be {0}\nbut is{1}".format(
                                self.imgstack_right_layer1.shape,
                                self.imgstack_right_layer2.shape,
                            ),
                        )
                        self.img_right_layer2, self.sceneRight.imagetype_layer2, self.imgstack_right_layer2 = (
                            None,
                        ...
                        self.imgstack_right_layer1 is None
                        and self.img_right_layer2.shape != self.img_right_layer1.shape
                    ):
                        QtGui.QMessageBox.critical(
                            self,
                            "Warning",
            ...
            if self.label_selimg.text() == "left":
                ...
                if (
                    self.img_left_layer3 is None
                    and self.checkBox_layer3.isChecked()
                    or load is True
                ):
                    path = str(
                        QtGui.QFileDialog.getOpenFileName(
                    ...
                    if path == "":
                        if load is True:
                            ...
                            return
                    # path = '/Users/jan/Desktop/correlation_test_dataset/single_tif_files/single_tif_files_1.tif'
                    self.img_left_layer3, self.sceneLeft.imagetype_layer3, self.imgstack_left_layer3 = self.imread(
                        path
                    )
                    ...
                    if self.sceneLeft.imagetype_layer3 != self.sceneLeft.imagetype:
                        ...
                        self.img_left_layer3, self.sceneLeft.imagetype_layer3, self.imgstack_left_layer3 = (
                            None,
                            None,
                            None,
                        )
                        ...
                        self.imgstack_left_layer1 is not None
                        and self.imgstack_left_layer3.shape
                        != self.imgstack_left_layer1.shape
                    ):
                        QtGui.QMessageBox.critical(
                            self,
                            ...
                            "This image file does not have the same dimensions as the first image: \nNeeds to be {0}\nbut is{1}".format(
                                self.imgstack_left_layer1.shape,
                                self.imgstack_left_layer3.shape,
                            ),
                        )
                        self.img_left_layer3, self.sceneLeft.imagetype_layer3, self.imgstack_left_layer3 = (
                            None,
                        ...
                        self.imgstack_left_layer1 is None
                        and self.img_left_layer3.shape != self.img_left_layer1.shape
                    ):
                        QtGui.QMessageBox.critical(
                            self,
                            "Warning",
                ...
                if (
                    ...
                    if path == "":
                        if load is True:
                            ...
                            return
                    # path = '/Users/jan/Desktop/correlation_test_dataset/single_tif_files/single_tif_files_1.tif'
                    self.img_right_layer3, self.sceneRight.imagetype_layer3, self.imgstack_right_layer3 = self.imread(
                        path
                    )
                    ...
                    if self.sceneRight.imagetype_layer3 != self.sceneRight.imagetype:
                        ...
                        self.img_right_layer3, self.sceneRight.imagetype_layer3, self.imgstack_right_layer3 = (
                            None,
                            None,
                            None,
                        )
                        ...
                        self.imgstack_right_layer1 is not None
                        and self.imgstack_right_layer3.shape
                        != self.imgstack_right_layer1.shape
                    ):
                        QtGui.QMessageBox.critical(
                            self,
                            ...
                            "This image file does not have the same dimensions as the first image: \nNeeds to be {0}\nbut is{1}".format(
                                self.imgstack_right_layer1.shape,
                                self.imgstack_right_layer3.shape,
                            ),
                        )
                        self.img_right_layer3, self.sceneRight.imagetype_layer3, self.imgstack_right_layer3 = (
                            None,
                        ...
                        self.imgstack_right_layer1 is None
                        and self.img_right_layer3.shape != self.img_right_layer1.shape
                    ):
                        QtGui.QMessageBox.critical(
                            self,
                            "Warning",
    ...
    def exportPoints(self):
        ...
        csv_file_out, filterdialog = QtGui.QFileDialog.getSaveFileNameAndFilter(
            self,
            "Export file as",
            os.path.dirname(self.leftImage)
            if side == "left"
            else os.path.dirname(self.rightImage),
    ...
    def importPoints(self):
        ...
        self.activateWindow()
        if str(filterdialog).startswith("Comma") is True:
            itemlist = csvHandler.csv2list(
                csv_file_in, delimiter=",", parent=self, sniff=True
            )
        elif str(filterdialog).startswith("Tabstop") is True:
            itemlist = csvHandler.csv2list(
                csv_file_in, delimiter="\t", parent=self, sniff=True
    ...
    def model2np(self, model, rows):
        ...
        for rowNumber in range(*rows):
            fields = [
                model.data(
                    model.index(rowNumber, columnNumber), QtCore.Qt.DisplayRole
                ).toFloat()[0]
    ...
    def correlate(self):
        if (
            "{0:b}".format(self.sceneLeft.imagetype)[-1] == "1"
            and "{0:b}".format(self.sceneRight.imagetype)[-1] == "0"
        ):
            ...
            "{0:b}".format(self.sceneLeft.imagetype)[-1] == "0"
            and "{0:b}".format(self.sceneRight.imagetype)[-1] == "1"
        ):
            ...
            def corrMsgBox(self, msg):
                ...
                msgBox.setText(msg)
                l2rButton = msgBox.addButton(
                    "Left to Right", QtGui.QMessageBox.ActionRole
                )
                r2lButton = msgBox.addButton(
                    "Right to Left", QtGui.QMessageBox.ActionRole
                )
            ...
            if (
                ...
                if rowsLeft > rowsRight:
                    ...
                    corrMsgBoxRetVal = corrMsgBox(
                        self,
                        "It seems you want to do a 3D to 3D correlation. "
                ...
                if rowsLeft > rowsRight:
                    ...
                    corrMsgBoxRetVal = corrMsgBox(
                        self,
                        "It seems you want to do a 2D to 2D correlation. "
                        + "Since both data sets contain the same amount of markers, please specify which side you want to correlate to:",
                    )
                if corrMsgBoxRetVal == "l2r":
                    model2D = self.modelRight
                ...
                QtGui.QMessageBox.critical(
                    self, "Data Structure", "Cannot determine if datasets are 2D or 3D"
                )
        ...
        if nrRowsModel2D >= 3:
            if nrRowsModel2D <= nrRowsModel3D:
                ...
                QtGui.QMessageBox.critical(
                    self,
                    "Data Structure",
                    "The two datasets do not contain the same amount of markers!",
                )
                return
        else:
            QtGui.QMessageBox.critical(
                self,
                "Data Structure",
                "At least THREE markers are needed to do the correlation",
            )
            return
        ...
        for i in range(transf_3d.shape[1]):
            cv2.circle(
                img,
                (int(round(transf_3d[0, i])), int(round(transf_3d[1, i]))),
                radius,
                self.markerColor,
                -1,
            )
        ...
        img_overlay = cv2.addWeighted(
            img_overlay, alpha, np.zeros(img.shape, dtype=img.dtype), 1 - alpha, 0.0
        )
        img_orig = np.copy(img)
            ...
            for i in range(calc_spots_2d.shape[1]):
                if poiForm == 0:
                    ...
                    cv2.line(
                        img,
                        (
                            int(round(calc_spots_2d[0, i] - poiCrossLength)),
                            int(round(calc_spots_2d[1, i])),
                        ),
                        (
                            int(round(calc_spots_2d[0, i] + poiCrossLength)),
                            int(round(calc_spots_2d[1, i])),
                    ...
                    cv2.line(
                        img,
                        (
                            int(round(calc_spots_2d[0, i])),
                            int(round(calc_spots_2d[1, i]) - poiCrossLength),
                        ),
                        (
                            int(round(calc_spots_2d[0, i])),
                            int(round(calc_spots_2d[1, i]) + poiCrossLength),
        ...
        img_overlay = cv2.addWeighted(
            img_overlay, poiAlpha, img_overlay_orig, 1 - poiAlpha, 0.0
        )
        if self.checkBox_writeReport.isChecked():
            cv2.imwrite(
                os.path.join(self.workingdir, timestamp + "_correlated.tif"), img
            )
    ...
    def displayResults(self, frame=False, framesize=None):
        ...
        if hasattr(self, "correlation_results"):
            ...
            self.label_translation.setText(
                "x = {0:.3f} | y = {1:.3f}".format(translation[0], translation[1])
            )
            self.label_translation.setStyleSheet(self.stylesheet_green)
            self.label_custom_rot_center.setText(
                "[{0},{1},{2}]:".format(
            ...
            self.label_translation_custom_rot.setText(
                "x = {0:.3f} | y = {1:.3f}".format(
            ...
            self.label_meandxdy.setText(
                "{0:.5f} / {1:.5f}".format(delta2D_mean[0], delta2D_mean[1])
            )
            ...
            self.label_rms.setStyleSheet(
                self.stylesheet_green if transf.rmsError < 1 else self.stylesheet_orange
            )

            self.widget_matplotlib.setupScatterCanvas(
                width=4, height=4, dpi=52, toolbar=False
            )
    ...
    def showSelectedResidual(self, doubleclick=False):
        ...
        if indices:
            ...
            for row in rows:
                markerNr = (
                    int(
                        self.modelResultsProxy.data(
                            self.modelResultsProxy.index(row, 0)
                        ).toString()
                    )
                    - 1
                )
        ...
        if doubleclick is True:
            ...
            if debug is True:
                print(
                    ...
                    graphicsView.transform().m22(),
                )
            graphicsView.setTransform(
                QtGui.QTransform(
                    20,  # m11
            ...
            graphicsView.centerOn(
                float(
                    tableView1._model.data(
                        tableView1._model.index(markerNr, 0)
                    ).toString()
                ),
                float(
                    tableView1._model.data(
                        tableView1._model.index(markerNr, 1)
                    ).toString()
                ),
            )
    ...
    def applyResidualShift(self):
        ...
        if indices:
            ...
            for row in rows:
                ...
                if debug is True:
                    print(
                        clrmsg.DEBUG + "Marker number/background color (Qrgba)",
                        markerNr,
                        self.modelResults.itemFromIndex(
                            self.modelResultsProxy.mapToSource(
                                (self.modelResultsProxy.index(row, 0))
                            )
                        )
                        .background()
                ...
                if (
                    ...
                    items[markerNr].setPos(
                        float(
                            tableView._model.data(
                                tableView._model.index(markerNr, 0)
                            ).toString()
                        )
                        + self.correlation_results[3][0, markerNr],
                        float(
                            tableView._model.data(
                                tableView._model.index(markerNr, 1)
                            ).toString()
                        )
                        + self.correlation_results[3][1, markerNr],
                    )
                    self.modelResults.itemFromIndex(
                        self.modelResultsProxy.mapToSource(
                            (self.modelResultsProxy.index(row, 0))
                        )
                    ).setBackground(QtGui.QColor(*BackColor))
                    self.modelResults.itemFromIndex(
                        self.modelResultsProxy.mapToSource(
                            (self.modelResultsProxy.index(row, 1))
                        )
                    ).setBackground(QtGui.QColor(*BackColor))
                    self.modelResults.itemFromIndex(
                        self.modelResultsProxy.mapToSource(
                            (self.modelResultsProxy.index(row, 2))
                        )
                    ).setBackground(QtGui.QColor(*BackColor))
                    self.modelResults.itemFromIndex(
                        self.modelResultsProxy.mapToSource(
                    ...
                    ).setForeground(QtGui.QColor(*ForeColor))
                    self.modelResults.itemFromIndex(
                        self.modelResultsProxy.mapToSource(
                            (self.modelResultsProxy.index(row, 1))
                        )
...
class Main:
    def __init__(
        self, leftImage=None, rightImage=None, nosplash=False, workingdir=None
    ):
        ...
        if leftImage is None or rightImage is None:
            sys.exit(
                "Please pass 'leftImage=PATH' and 'rightImage=PATH' to this function"
            )
...
if __name__ == "__main__":
    ...
    left = str(
        QtGui.QFileDialog.getOpenFileName(
            None,
            "Select first image file for correlation",
            execdir,
            "Image Files (*.tif *.tiff);; All (*.*)",
        )
    )
    ...
    right = str(
        QtGui.QFileDialog.getOpenFileName(
            None,
            "Select second image file for correlation",
            execdir,
            "Image Files (*.tif *.tiff);; All (*.*)",
        )
    )
