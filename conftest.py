...
def image_RGB(tmpdir_factory):
    ...
    tf.imsave(str(fn), img.astype('uint8'), metadata={
              "PhysicalSizeX": "123", "FocusStepSize": "456"})
    return fn
...
def image_Grey(tmpdir_factory):
    ...
    tf.imsave(str(fn), img.astype('uint8'), metadata={
              "PhysicalSizeZ": "0.123", "FIBimage": "PixelWidth=4.56e-006\r\n"})
    return fn
...
def insertSphere(volIn, pos, radius, blur=False):
    try:
        if pos[0] < radius or pos[0] > volIn.shape[0] - radius:
            raise Exception(
                "Sphere's x position too close to volume boundaries! Returning original volume...")
        if pos[1] < radius or pos[1] > volIn.shape[1] - radius:
            raise Exception(
                "Sphere's y position too close to volume boundaries! Returning original volume...")
        if pos[2] < radius or pos[2] > volIn.shape[2] - radius:
            raise Exception(
                "Sphere's z position too close to volume boundaries! Returning original volume...")
    ...
    volIn[pos[0]-r:pos[0]-r+d, pos[1]-r:pos[1] -
          r+d, pos[2]-r:pos[2]-r+d] += sphere
