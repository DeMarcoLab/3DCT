...
class APP(QtGui.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.actionHelp.setStatusTip(
            "Open User's Guide: http://3dct.semper.space/userguide.html"
        )
    ...
    def isValidPath(self, lineEdit):
        ...
        if lineEdit.text() == "":
            ...
            if lineEdit.objectName() == "lineEdit_WorkingDirPath":
                workingdir = self.checkDirectoryPrivileges(
                    str(self.lineEdit_WorkingDirPath.text())
                )
                if workingdir:
                    lineEdit.setStyleSheet(
                        "QLineEdit{background-color: rgb(0,255,0,80);}\
    ...
    def checkDirectoryPrivileges(
        self, path, question="Do you want to select another directory?"
    ):
        """
        ...
        try:
            ...
            reply = QtGui.QMessageBox.critical(
                self,
                "Warning",
                "I cannot write to the folder: {0}\n\nDo you want to select another directory?".format(
                    path
                ),
    ...
    def openDirectoy(self, path):
        ...
        if debug is True:
            print(clrmsg.DEBUG, "os split (directory, file):", directory, file)
    ...
    def selectPath(self, pathLine):
        ...
        path = str(
            QtGui.QFileDialog.getExistingDirectory(
                self, "Select Directory", self.workingdir
            )
        )
        if path:
        ...
        if sender == self.toolButton_ImageStackGetPixelSize:
            try:
                pixelSizeXY = stackProcessing.pxSize(
                    str(self.lineEdit_ImageStackPath.text())
                )
                pixelSizeZ = stackProcessing.pxSize(
                    str(self.lineEdit_ImageStackPath.text()), z=True
                )
                if debug is True:
                    print(clrmsg.DEBUG + "Pixelsize xy/z", pixelSizeXY, pixelSizeZ)
                if pixelSizeXY:
                    self.doubleSpinBox_ImageStackFocusStepSizeReslized.setValue(
                        pixelSizeXY * 1000
                    )
                else:
                    raise Exception("No xy pixel size information found!")
                if pixelSizeZ:
                    self.doubleSpinBox_ImageStackFocusStepSizeOrig.setValue(
                        pixelSizeZ * 1000
                    )
                else:
                ...
                QtGui.QMessageBox.warning(
                    self, "Warning", "Unable to extract pixel size.\n\n{0}".format(e)
                )
        elif sender == self.toolButton_ImageSequenceGetPixelSize:
            try:
                print(
                    os.path.join(
                        str(self.lineEdit_ImageSequencePath.text()),
                        "Tile_001-001-000_0-000.tif",
                    )
                )
                pixelSizeXY = stackProcessing.pxSize(
                    os.path.join(
                ...
                if debug is True:
                    print(clrmsg.DEBUG + "Pixelsize xy/z", pixelSizeXY, pixelSizeZ)
                if pixelSizeXY:
                    self.doubleSpinBox_ImageSequenceFocusStepSizeReslized.setValue(
                        pixelSizeXY * 1000
                    )
                else:
                    raise Exception("No xy pixel size information found!")
                if pixelSizeZ:
                    self.doubleSpinBox_ImageSequenceFocusStepSizeOrig.setValue(
                        pixelSizeZ * 1000
                    )
                else:
                    raise Exception("No focus step size information found!")
    ...
    def populate_filelist(self, path):
        ...
        for fname in sorted(os.listdir(path)):
            ...
            if (
                os.path.isdir(checkdir) is False
                and fname.startswith(".") is False
                and fname.startswith("$") is False
            ):
    ...
    def reloadFileList(self):
        ...
        if hasattr(self, "workingdir"):
            self.populate_filelist(self.workingdir)
            if debug is True:
                print(clrmsg.DEBUG, "Working directory file list reloaded")
    ...
    def selectImage1(self):
        ...
        self.lineEdit_selectImage1.setText(
            os.path.join(
                self.listWidget_WorkingDir.itempath,
                str(self.listWidget_WorkingDir.selectedItems()[0].text()),
            )
        )
    ...
    def runCorrelationModule(self):
        ...
        if hasattr(self, "correlationModul"):
            if hasattr(self.correlationModul, "window"):
                ...
                return
        if (
            self.lineEdit_selectImage1.text() != ""
            and self.lineEdit_selectImage2.text() != ""
        ):
            if (
                self.lineEdit_selectImage1.fileIsTiff is True
                and self.lineEdit_selectImage2.fileIsTiff is True
            ):
                ...
                if (
                    self.lineEdit_selectImage1.fileIsValid is False
                    or self.lineEdit_selectImage2.fileIsValid is False
                ):
                    QtGui.QMessageBox.warning(
                        self,
                        "Warning",
                    ...
                    self.lineEdit_selectImage1.fileIsTiff is False
                    or self.lineEdit_selectImage2.fileIsTiff is False
                ):
                    QtGui.QMessageBox.warning(
                        self,
                        "Warning",
    ...
    def imageStack(self):
        ...
        if (
            img_path
            and self.lineEdit_ImageStackPath.fileIsTiff is True
            and customSaveDir
        ):
            ...
            if debug is True:
                print(clrmsg.DEBUG, img_path, ss_in, ss_out, customSaveDir)
            ...
            stackProcessing.main(
                ...
                ss_in,
                ss_out,
                qtprocessbar=self.progressBar_ImageStack,
    ...
    def imageSequence(self):
        ...
        customSaveDir = self.checkDirectoryPrivileges(
            dirPath, question="Do you want me to save the data to another directory?"
        )
            ...
            if self.checkBox_ImageSequenceCube.isChecked():
                ...
                if debug is True:
                    print(
                        ...
                        ss_in,
                        ss_out,
                        str(self.checkBox_ImageSequenceSaveOrigStack.isChecked()),
                        customSaveDir,
                    )
                ...
                stackProcessing.main(
                    ...
                    interpolationmethod="linear",
                    saveorigstack=self.checkBox_ImageSequenceSaveOrigStack.isChecked(),
                    showgraph=False,
                    customSaveDir=customSaveDir,
                )
                ...
                if debug is True:
                    print(clrmsg.DEBUG, "no reslicing")
                self.progressBar_ImageSequence.setMaximum(100)
                QtGui.QApplication.processEvents()
                stackProcessing.main(
                    dirPath,
    ...
    def normalize(self):
        ...
        if (
            img_path
            and self.lineEdit_NormalizePath.fileIsTiff is True
            and customSaveDir
        ):
            if debug is True:
                print(clrmsg.DEBUG, "In/out:", img_path, customSaveDir)
            self.progressBar_Normalize.setMaximum(100)
            QtGui.QApplication.processEvents()
            stackProcessing.normalize(
                img_path,
                qtprocessbar=self.progressBar_Normalize,
                customSaveDir=customSaveDir,
            )
        ...
        if img_path and self.lineEdit_MipPath.fileIsTiff is True and customSaveDir:
            if debug is True:
                print(
                    ...
                    img_path,
                    customSaveDir,
                    self.checkBox_MipNormalize.isChecked(),
                )
            self.progressBar_Mip.setMaximum(100)
            QtGui.QApplication.processEvents()
            stackProcessing.mip(
                img_path,
                qtprocessbar=self.progressBar_Mip,
                customSaveDir=customSaveDir,
                normalize=self.checkBox_MipNormalize.isChecked(),
            )
...
if __name__ == "__main__":
    if debug is True:
        ...
        print(
            clrmsg.WARNING
            + "Debug mode can/will slow down parts of the Toolbox (e.g. marker clicking)"
        )
