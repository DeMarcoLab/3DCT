...
def getzPoly(x, y, img, n=None, optimize=False):
    ...
    if optimize is True:
        x_opt_vals, y_opt_vals, z_opt_vals = optimize_z(
            x, y, data_z_xp_poly, img, n=None
        )
...
def getzGauss(
    x, y, img, parent=None, optimize=False, threshold=None, threshVal=0.6, cutout=15
):
    """x and y are coordinates
        img is the path to the z-stack tiff file or a numpy.ndarray from tifffile.py imread function
    ...
    if optimize is False:
        ...
        if clrmsg and debug is True:
            print(
                clrmsg.DEBUG
                + "2D Gaussian xy optimization running %.f at z = %.f"
                % (repeats, round(poptZ[1]))
            )
...
def optimize_z(x, y, z, image, n=None):
    ...
    for i in range(5):
        try:
            ...
            x_opt, y_opt = optimize_xy(
                x_opt, y_opt, z_opt, img, nx=None, ny=None)
            ...
            print(
                IndexError(
                    "Optimization failed, possibly due to low signal or low SNR. "
                    + str(e)
                )
            )
            return [x], [y], ["failed"]
        n = getn(data_z)
        z_opt, data_z_yp_poly = parabolic.parabolic_polyfit(
            data_z, np.argmax(data_z), n
        )
...
def gaussfit(data, parent=None, hold=False):
    ...
    if parent is not None:
        ...
        if hold is False:
            parent.widget_matplotlib.setupScatterCanvas(
                width=4, height=4, dpi=52, toolbar=False
            )
        parent.widget_matplotlib.xyPlot(
            data[0], data[1], label="z data", clear=True)
        parent.widget_matplotlib.xyPlot(
            x, y, label="gaussian fit", clear=False)
    ...
    if clrmsg and debug is True:
        ...
        print(
            clrmsg.DEBUG + "FWHM            :", popt[2] *
            2 * math.sqrt(2 * math.log(2, math.e))
        )
        print(clrmsg.DEBUG + "Std. Amplitude    :", std_height)
        print(clrmsg.DEBUG + "Std. Location    :", std_mean)
        print(
            clrmsg.DEBUG + "Std. FWHM        :",
            std_sigma * 2 * math.sqrt(2 * math.log(2, math.e)),
        )
        print(clrmsg.DEBUG + "Mean dy        :",
              np.absolute(y - data[1]).mean())
...
def moments(data):
    ...
    y = (Y * data).sum() / total
    col = data[:, int(y)]
    width_x = np.sqrt(abs((np.arange(col.size) - y)
                          ** 2 * col).sum() / col.sum())
    row = data[int(x), :]
    width_y = np.sqrt(abs((np.arange(row.size) - x)
                          ** 2 * row).sum() / row.sum())
    height = data.max()
...
def fitgaussian(data, parent=None):
    ...
    if np.isnan(p).any():
        parent.widget_matplotlib.matshowPlot(
            mat=data,
            contour=np.ones(data.shape),
            labelContour="XY optimization failed\n"
    ...
    if parent is not None:
        ...
        parent.widget_matplotlib.matshowPlot(
            mat=data, contour=contour, labelContour=labelContour
        )
    return p
