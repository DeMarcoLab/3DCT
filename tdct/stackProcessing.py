...
def main(
    ...
    if interpolationmethod == "none" and saveorigstack is False and showgraph is False:
        print(
            clrmsg.WARNING,
            "At least let me do something! Setting everything to False... very funny -.-",
        )
        return
    ## For single image stack files
    if os.path.isfile(img_path) is True:
        if debug is True:
            print(clrmsg.DEBUG, "Loading image: ", img_path)
        if qtprocessbar:
        ...
        if len(img.shape) < 3:
            print(
                clrmsg.ERROR,
                "ERROR: This seems to be a 2D image with the shape {0}. Please select a stack image file.".format(
                    img.shape
                ),
            )
            return
        if debug is True:
            print(clrmsg.DEBUG, "        ...done.")
        ## Get pixel size
        if qtprocessbar:
        ...
        try:
            ...
            if pixelsize is not None:
                px_info = True
                if debug is True:
                    print(clrmsg.DEBUG, "Adding pixel size information:", pixelsize)
            else:
                px_info = False
        except Exception as e:
            print(
                clrmsg.ERROR,
                "Error while adding pixel size information:",
                e,
                "... skipping",
            )
            px_info = False
        ## Start Processing
        if debug is True:
            print(clrmsg.DEBUG, px_info)
        if qtprocessbar:
            qtprocessbar.setValue(60)
            QtGui.QApplication.processEvents()
        if customSaveDir:
            file_out_int = os.path.join(
                customSaveDir,
                os.path.splitext(os.path.split(img_path)[1])[0] + "_resliced.tif",
            )
        else:
            file_out_int = os.path.join(
                img_path, os.path.splitext(img_path)[0] + "_resliced.tif"
            )  # revisit
        if debug is True:
            print(clrmsg.DEBUG, "Interpolating...")
        ...
        if type(img_int) == str:
            if debug is True:
                print(clrmsg.DEBUG, img_int)
            return
        if img_int is not None:
            if debug is True:
                print(clrmsg.DEBUG, "Saving interpolated stack as: ", file_out_int)
            if px_info is True:
                tf.imsave(
                    file_out_int,
                    img_int,
    ...
    ## For image sequence (only FEI MAPS/LA image sequences at the moment)
        ...
        if debug is True:
            print(clrmsg.DEBUG, "Checking directory: ", img_path)
        channels = []
        ...
        for filename in files:
            if fnmatch.fnmatch(filename, "Tile_*.tif"):
                ...
                channels.append(
                    filename[17]
                )  # 'Tile_001-001-001_1-000.tif' 17th character is the amount of channels
        if match is False:
            print(
                clrmsg.ERROR,
                (
        ...
        for i in range(channels):
            ...
            if debug is True:
                print(
                    clrmsg.DEBUG,
                    "Processing channel {0} of {1}".format(i + 1, channels),
                )
            filelist = []
            ...
            if customSaveDir:
                file_out_int = os.path.join(
                    customSaveDir,
                    os.path.basename(os.path.normpath(img_path))
                    + "_"
                    + str(i)
                    + "_resliced.tif",
                )
            ...
            if saveorigstack is True:
                ...
                if debug is True:
                    print(
                        clrmsg.DEBUG,
                        "Saving original image stack as single stack file: {0} |shape: {1}".format(
                            file_out_orig, img.shape
                        ),
                    )
                if px_info is True:
                    tf.imsave(
            ...
            if interpolationmethod == "none" and showgraph is False:
                ...
                if type(img_int) == str:
                    ...
                    if debug is True:
                        print(
                            clrmsg.DEBUG, "Saving interpolated stack as: ", file_out_int
                        )
                    if px_info is True:
                        tf.imsave(
                            file_out_int,
        ...
        print(
            clrmsg.ERROR, "ERROR: Path is neither a valid file nor a valid directory!"
        )
...
def interpol(img, ss_in, ss_out, interpolationmethod, showgraph):
    ...
    if interpolationmethod == "none":
        ...
        if debug is True:
            print(clrmsg.DEBUG, "Nr. of slices (in/out): ", sl_in, sl_out)
...
def spline(img, img_int_shape, ss_in, ss_out, sl_in, sl_out):
    ...
    if debug is True:
        print(clrmsg.DEBUG, "Interpolated stack shape: ", img_int.shape)
    ...
    pong = time.time()
    if debug is True:
        print(clrmsg.DEBUG, "This interpolation took {0} seconds".format(pong - ping))
    return img_int
...
def norm_img(img, copy=False, qtprocessbar=None):
    ...
    if debug is True:
        print(clrmsg.DEBUG, "Shape/type:", img.shape, dtype)
    ## 2D image
    if img.ndim == 2:
        if debug is True:
    ...
    ## 3D and multichannel image
        ...
        if debug is True:
            print(clrmsg.DEBUG, "3D and multichannel image")
        if qtprocessbar:
...
def normalize(path, qtprocessbar=None, customSaveDir=None):
    if debug is True:
        print(clrmsg.DEBUG, "Normalizing:", path)
    img = tf.imread(path)
    if qtprocessbar:
        qtprocessbar.setValue(10)
        QtGui.QApplication.processEvents()
    ...
    if debug is True:
        print(clrmsg.DEBUG, "        ...done")
    if debug is True:
        print(clrmsg.DEBUG, "Finished normalizing.")
...
def mip(path, qtprocessbar=None, customSaveDir=None, normalize=False):
    if debug is True:
        print(
            clrmsg.DEBUG,
            "Creating normalized Maximum Intensity Projection (MIP):",
            path,
        )
    img = tf.imread(path)
    if qtprocessbar:
    ...
    if len(img.shape) == 4:
        ...
        if normalize:
            if debug is True:
                print(clrmsg.DEBUG, "Normalizing...")
            img = norm_img(img)
        if debug is True:
            print(clrmsg.DEBUG, "Saving...")
        ...
        print(
            clrmsg.ERROR,
            "I'm sorry, I don't know this image shape: {0}".format(img.shape),
        )
...
if __name__ == "__main__":
    ...
    pb_hd = tkinter.ttk.Progressbar(
        ft, orient="horizontal", mode="indeterminate", length=700
    )
    ...
    def getfiles():
        files = tkinter.filedialog.askopenfilenames(
            parent=root, title="Choose image stack files"
        )
        if not files:
            return
        filenames = []
        ...
        ss_in = tkinter.simpledialog.askfloat(
            parent=root,
            title="Enter ORIGINAL focus step size",
        ...
        ss_out = tkinter.simpledialog.askfloat(
            parent=root,
            title="Enter INTERPOLATED focus step size",
            prompt="Enter INTERPOLATED focus step size for:\n"
            + "\n".join("{}".format(k) for k in filenames),
            initialvalue=pixelsize * 1000,
        )
        if not ss_out:
            return
        print(
        ...
        for filename in files:
            main(
                ...
                ss_out,
                saveorigstack=False,
                interpolationmethod=int_method.get(),
                showgraph=bool(showgraph.get()),
            )
    ...
    def getdirint():
        directory = tkinter.filedialog.askdirectory(
            parent=root, title="Choose directory with image sequence stack files"
        )
        if not directory:
            return
        ss_in = tkinter.simpledialog.askfloat(
        ...
        if not ss_out:
            return
        saveorigstack = tkinter.messagebox.askyesno(
            "Save single stack file option",
            "Do you also want to save single stack file with original focus step size?",
        )
        print(
            "directory: {0}\n".format(directory),
            "Focus step size in: {0} | out: {1}\n".format(ss_in, ss_out),
    ...
    def normalize():
        files = tkinter.filedialog.askopenfilenames(
            parent=root, title="Choose image(stack) file(s)"
        )
        if not files:
            return
        for filename in files:
    ...
    def mip():
        ...
        for filename in files:
            if filename.endswith(".tif"):
                print(
                    "Creating normalized Maximum Intensity Projection (MIP):", filename
                )
                img = tf.imread(filename)
                fpath, fname = os.path.split(filename)
                ...
                if len(img.shape) == 4:
                    img_MIP = np.zeros(
                        (img.shape[0], img.shape[2], img.shape[3]), dtype=img.dtype
                    )
                    ...
                    print(
                        "I'm sorry, I don't know this image shape: {0}".format(
                            img.shape
                        )
                    )
                print("        ...done")
    ...
    ### Buttons
    B1 = tkinter.Button(
        root, text="Interpolate single stack file(s)...", command=getfiles
    )
    B1.config(width=50)
    ...
    B3 = tkinter.Button(
        root,
        text="Just convert image stack sequence to single stack files...",
        command=getdircon,
    )
    B3.config(width=50)
    ...
    B5 = tkinter.Button(
        root, text="Create normalized MIP of image stack files...", command=mip
    )
    B5.config(width=50)
    B5.grid(row=9, column=0, columnspan=2)
    ### Check-boxes
    c = tkinter.Checkbutton(
        root, text="Show graph comparing interpolation methods", variable=showgraph
    )
