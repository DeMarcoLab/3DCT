...
class QTableViewCustom(QtGui.QTableView):
    ...
    def updateItems(self):
        ...
        if debug is True:
            print(
                clrmsg.DEBUG + "Update items check - Nr. of items/rows:",
                len(items),
        ...
        if len(items) == self._model.rowCount():
            ...
            for item in items:
                ...
                self._scene.zValuesDict[item] = [
                    self._model.data(self._model.index(row, 2)).toString(),
                    self._model.itemFromIndex(self._model.index(row, 2))
                    .foreground()
                    .color()
                    .getRgb(),
                ]
    ...
    def contextMenuEvent(self, event):
        ...
        if indices:
            ...
            cmGetZgaussOptL1.triggered.connect(
                lambda: self.getz(self.img1, gauss=True, optimize=True)
            )
            # Layer 2
            ...
            cmGetZgaussOptL2.triggered.connect(
                lambda: self.getz(self.img2, gauss=True, optimize=True)
            )
            # Layer 3
            ...
            cmGetZgaussOptL3.triggered.connect(
                lambda: self.getz(self.img3, gauss=True, optimize=True)
            )
            ...
            for row in rows:
                ...
                if gauss is True:
                    if optimize is False:
                        ...
                        if debug is True:
                            print(clrmsg.DEBUG + str(img.shape), zopt)
                        if 0 <= zopt <= img.shape[-3]:
                            self._scene.zValuesDict[activeitems[row]][1] = (0, 0, 0)
                            self._model.itemFromIndex(
                                self._model.index(row, 2)
                            ).setForeground(QtCore.Qt.black)
                        else:
                            self._scene.zValuesDict[activeitems[row]][1] = (255, 0, 0)
                            self._model.itemFromIndex(
                                self._model.index(row, 2)
                            ).setForeground(QtCore.Qt.red)
                        self._model.itemFromIndex(self._model.index(row, 2)).setText(
                            str(zopt)
                        )
                    else:
                        xopt, yopt, zopt = beadPos.getzGauss(
                            ...
                            y,
                            img,
                            parent=self.mainParent,
                            optimize=True,
                            threshold=True,
                            threshVal=self.mainParent.doubleSpinBox_treshVal.value(),
                            cutout=self._scene.markerSize,
                        )
                        if debug is True:
                            print(clrmsg.DEBUG + str(img.shape), xopt, yopt, zopt)
                        if (
                            abs(x - xopt) <= 2 * self._scene.markerSize
                            and abs(y - yopt) <= 2 * self._scene.markerSize
                            and 0 <= zopt <= img.shape[-3]
                        ):
                            self._scene.zValuesDict[activeitems[row]][1] = (255, 0, 0)
                            self._model.itemFromIndex(
                                self._model.index(row, 2)
                            ...
                            xopt, yopt = x, y
                        self._model.itemFromIndex(self._model.index(row, 0)).setText(
                            str(xopt)
                        )
                        self._model.itemFromIndex(self._model.index(row, 1)).setText(
                            str(yopt)
                        )
                        self._model.itemFromIndex(self._model.index(row, 2)).setText(
                            str(zopt)
                    ...
                    xopt, yopt, zopt = beadPos.getzPoly(
                        x, y, img, n=None, optimize=True
                    )
                    if debug is True:
                        print(clrmsg.DEBUG + str(img.shape), xopt, yopt, zopt)
                    if (
                        0 <= xopt <= img.shape[-1]
                        and 0 <= yopt <= img.shape[-2]
                        and 0 <= zopt <= img.shape[-3]
                    ):
                        self._scene.zValuesDict[activeitems[row]][1] = (255, 0, 0)
                        self._model.itemFromIndex(
...
class QGraphicsSceneCustom(QtGui.QGraphicsScene):
        ...
        if (
            ...
            if self.mainWidget.checkBox_MIP.isChecked():
                ...
                self.addCircle(
                    event.scenePos().x(),
                    event.scenePos().y(),
                    z=self.mainWidget.spinBox_slice.value(),
                )
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.selectedItems() and self.selectionmode is False:
            if debug is True:
                print(
                    clrmsg.DEBUG + "New pos:",
                    self.selectedItems()[0].x(),
                    self.selectedItems()[0].y(),
                )
            ...
            if "{0:b}".format(self.imagetype)[-1] == "0":
                for item in self.selectedItems():
                    if isinstance(item, QtGui.QGraphicsEllipseItem):
                        self.zValuesDict[item] = [
                            self.zValuesDict[item][0],
                            (255, 190, 0),
                        ]  # orange
            self.clearSelection()
            self.itemsToModel()
    ...
    def addCircle(self, x, y, z=None):
        ...
        QtGui.QGraphicsItem.stackBefore(circle, list(self.items())[-2])
        self.enumeratePoints()
    ...
    def enumeratePoints(self):
        ## Remove numbering
        for item in list(self.items()):
            if isinstance(item, QtGui.QGraphicsSimpleTextItem) or isinstance(
        ...
        for item in list(self.items()):
            if isinstance(item, QtGui.QGraphicsEllipseItem):
                ...
                ## Adding number
                nr = self.addSimpleText(
                    str(pointidx),
                    QtGui.QFont("Helvetica", pointSize=1.5 * self.markerSize),
                )
                ...
                nr.setPos(
                    math.cos(radangle) * self.markerSize,
                    math.sin(radangle) * self.markerSize,
                )
                ...
                hline.setPen(
                    QtGui.QPen(QtGui.QColor(255, 255, 255, 128))
                )  # r,g,b,alpha, white transparent
...
class MatplotlibWidgetCustom(QtGui.QWidget):
    ...
    def scatterPlot(
        self, x="random", y="random", frame=False, framesize=None, xlabel="", ylabel=""
    ):
...
class QLineEditFilePath(QtGui.QLineEdit):
        ...
        if sys.platform == "darwin":
            ...
            try:
                ...
                if debug is True:
                    print(
                        clrmsg.DEBUG
                        + """"objc" and "CoreFoundation" import successful | Drag'n'Drop support for Mac OS X >= 10.10"""
                    )
    ...
    def dropEvent(self, event):
        ...
        if urls and urls[0].scheme() == "file":
            ...
            if filepath.startswith(".file/id=") and objc:
                if debug is True:
                    print(clrmsg.DEBUG + "File id bug in PyQt4, converting:", filepath)
                filepath = str(self.getUrlFromLocalFileID(urls[0]))
                if debug is True:
                    print(clrmsg.DEBUG + "                            to ->", filepath)
    ...
    def getUrlFromLocalFileID(self, localFileID):
        ...
        relCFStringRef = CF.CFStringCreateWithCString(
            CF.kCFAllocatorDefault, localFileQString.toUtf8(), CF.kCFStringEncodingUTF8
        )
        ...
        absCFURL = CF.CFURLCreateFilePathURL(
            CF.kCFAllocatorDefault, relCFURL, objc.NULL
        )
